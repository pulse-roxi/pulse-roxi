...
def kalmanf(s):
    ...
    if "P" not in s:
        s["P"] = np.nan
    if "z" not in s:
        raise ValueError("Observation vector missing")
    if "u" not in s:
        s["u"] = 0
    if "A" not in s:
        s["A"] = np.eye(len(s["x"]))
    ...
    if "R" not in s:
        raise ValueError("Observation covariance missing")
    if "H" not in s:
        s["H"] = np.eye(len(s["x"]))
    if "V" not in s:
        s["V"] = np.eye(len(s["x"]))

    if np.all(np.isnan(s["x"])):
        # Initialize state estimate from first observation
        if np.diff(s["H"].shape):
            raise ValueError(
                "Observation matrix must be square and invertible for state initialization."
            )
        s["x"] = np.linalg.solve(s["H"], s["z"])
        s["P"] = np.linalg.solve(s["H"], s["R"]).dot(np.linalg.inv(s["H"].T))
        # s['x'] = np.linalg.inv(s['H']).dot(s['z'])
        # s['P'] = np.linalg.inv(s['H']).dot(s['R']).dot(np.linalg.inv(s['H'].T))
    else:
        # This is the code that implements the discrete Kalman filter
        ...
        denom = np.linalg.inv(s["H"] @ s["P"] @ (s["H"].T) + s["R"])
        K = s["P"] @ (s["H"].T) @ denom

        # Correction based on observation
        s["x"] = s["x"] + K @ (s["z"] - s["H"] @ s["x"] - s["V"])
        s["P"] = s["P"] - K @ s["H"] @ s["P"]
        ...
        # (That's how simple the discrete Kalman filter is to use.)
        # Later, we'll discuss how to deal with nonlinear systems.

        # Oliver Tracy's code changes line 179 to
        # s['x'] = s['x'] + K.dot(s['H'].dot(s['z']) - s['H'].dot(s['x']))
        # but the dimensions don't match up
...
# change log:
# 2021-07-30 -
#   Kenny Chou - changed 179 to K*(s.z-s.H*s.x-s.V) from K*(s.z-s.H*s.x)
#   Accoding to Khreis et al 2020, V_k = [b1_k b2_k]', where b_k is the
#   measurement noise, defined by the difference between the measurement
#   and the sinusoidal model. The subscript k denotes a unit in time.
# 2021 08-05
#   Changed line 166 and 167, and 177 from inv(A)*b to A\b and b*inv(A) to b/A
