...
def est_AUDP(ppg: np.ndarray, max_idxs: np.ndarray, min_idxs: np.ndarray):
    """Calculate the area under descending ppg pulse (AUDP), descriebd in Cernat 2015.

    Cernet, 2015 describes three features - audp area under the entire ppg pulse (AUP),
    area under ascending ppg pulse (AUAP), and area under descending ppg pulse (AUDO).
...
def analyse_pulse_features(ppg, min_idxs, show: bool=False):
    """Find the fiducial points in a ppg pulse, the derive features.
...
def extract_ppg_features(
    ...
    * Amplitude variations - PPG pulse amplitudes based on:
    ...
    # In order to identify various PPG pulses, there needs to be one maxima
    # that correspond to each minima.
    ...
    # NOTE: notch-related features may no be identifiable. Excluding these features
...
def cut_riv_outliers(
    ...
    * Only keep segments where segment length > min_keep_pct*signal_len
    ...
    Args:
        ...
        min_keep_pct: minimum percentage of frame to be kept. Between 0 and 1.
